...
- **Daily challenge mode** — seed the day's hard-mode spotting order from the date (Wordle-style), so every rider faces the same rainbow-order challenge and can compare results; pairs naturally with a "share your result" emoji grid (e.g. 🟥🟧🟨🟩🟦🟪) that encodes find-order without spoiling the answer.
...
- **Two-player race mode** — give each backseat rider their own checklist on the same device (or two tabs synced via `BroadcastChannel`) so siblings can race to spot all six colors out the window first; leans on the existing click/active-state model with minimal new mechanics.
...
A real-time, planet-wide shared checklist: every visitor across the globe is hunting the _same_ six rainbow colors out their car windows, and the instant _anyone, anywhere_ spots one, it lights up for _everyone_ simultaneously (via a lightweight WebSocket/pub-sub backend). Finishing the rainbow becomes a collective act — thousands of strangers on thousands of unrelated drives unknowingly cooperating to complete one giant, perpetually-resetting rainbow together, with a live counter showing how many times humanity has finished it. Equal parts beautiful and unhinged: it turns a 30-second backseat diversion into an ambient, ongoing piece of internet performance art that nobody agreed to participate in.
